...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:template match="/exportData">
		<fo:root>
			...
			<!-- <fo:page-sequence master-reference="Letter" font-family="Cyberbit,Arial,Helvetica"> -->
	...
	<xsl:template match="Book">
		<fo:block margin-top="0.0625in" margin-bottom="0.125in" keep-together="always">
	...
	<xsl:template match="field">
		<xsl:if test="text()!=''">
			...
			<xsl:if test="@name='illustrators'">
				<fo:block margin-left="0.25in">
					Illustrators: <xsl:value-of select="." />
				</fo:block>
			</xsl:if>
